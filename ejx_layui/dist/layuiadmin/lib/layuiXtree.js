layui.define(['form'],function(exports){
	...
	layuiXtree.prototype.Loading = function (options) {
	    ...
	    layui.$.ajax({
	    	...
	    	contentType: options.options || 'text/html',
	    	//data:options.params || '',
	    	dataType:'json',
	    	success:function(data){
	    		options.data = data;
	    		_this._dataJson = data;
	    		_this.Initial(options);
	    	},
	    	error:function(e){
	    		console.log(e);
	...
	layuiXtree.prototype.CreateCkAll = function () {
	    ...
	    if (_this._ckall) {
	       // _this._domStr += '<div class="layui-xtree-item">';
	        _this._domStr += '<div class="layui-xtree-checkAll">';
	    ...
	    if (d.length > 0) {
	        for (i in d) {
	            ...
	            if (d[i].children.length > 0)
	                _this._domStr += '<i class="layui-icon layui-xtree-icon" data-xtree="' + (d[i].spread ? '1' : '0') + '">' + (d[i].spread ? _this._iconOpen : _this._iconClose) + '</i>';
	            else {
	...
	layuiXtree.prototype.Rendering = function () {
	    ...
	    for (var i = 0; i < xtree_items.length; i++) {
	        if (xtree_items[i].parentNode == _this._container)
	            ...
	            if(!layui.$(xtree_items[i]).children('.layui-xtree-icon').attr('data-xtree'))xtree_items[i].style.display = 'none';
	        }
	    }
	    ...
	    for (var i = 0; i < xtree_icons.length; i++) {
	        ...
	        xtree_icons[i].style.color = (xtree_icons[i].getAttribute('data-xtree') == 1) ? _this._iconOpenColor : _this._iconCloseColor;
	        xtree_icons[i].style.cursor = "pointer";
	        //
	        if(xtree_icons[i].getAttribute('data-xtree') == 1){
	        	var _cur = xtree_icons[i];
	        	var xtree_chi = _cur.parentNode.childNodes;
	        	for (var j = 0; j < xtree_chi.length; j++) {
                    if (xtree_chi[j].getAttribute('class') == 'layui-xtree-item')
                        xtree_chi[j].style.display = 'block';
                }
	        	_cur.setAttribute('data-xtree', '1')
                _cur.innerHTML = _this._iconOpen;
	        	_cur.style.color = _this._iconOpenColor;
	        }
	...
	layuiXtree.prototype.GetAllChecked = function () {
		var _this = this,
		container ="#"+ _this._containerid + ' .layui-form-checked';
		var checkeds = $(container);
		var vals = [];
		$.each(checkeds,function(i,item){
			var temp = $(item).parent('.layui-xtree-item').children('input');
			vals.push(temp);
		})
		return vals;
	} 
	...
	var pMenu ,targetTree,thisIndex = [];
	layuiXtree.prototype.on = function(events,options){
		var _that = this;
		if (typeof (events) !== 'string') {
            console.log('layuiXtree error:事件名配置出错');
        }
        var lIndex = events.indexOf('(');
        var eventName = events.substr(0, lIndex);
        var filter = events.substring(lIndex + 1, events.indexOf(')'));
        if(eventName == 'menu'){
        	pMenu = layui.$('#'+filter+'-menu');
        	targetTree = layui.$('*[lay-filter = '+filter+']');
        	document.oncontextmenu = function(e){
        		if(e.button == 2){
        			...
        			if(pMenu.css('display') == 'none'){
        				var _this = e.target;
        				targetItem = _this;
        				var isTarget =  layui.$(_this).parents('*[lay-filter = '+filter+']')[0];
        				var parents = layui.$(_this).parents('.layui-xtree-item');
        				var indexDeep = parents.length;
        				var isTreeItem = parents[0];
        				thisIndex = [];
        				if(isTarget && _this.nodeName == 'SPAN' && isTreeItem){
        					pMenu.css({'left':(x+15)+'px','top':y+'px','display':'block'});
        					for(var i=0;i<indexDeep;i++){
        						isTreeItem= parents[i];
    							var items = layui.$(isTreeItem).parent().children('.layui-xtree-item');
            					layui.$.each(items,function(i){
            						if(this == isTreeItem){
            							thisIndex .unshift(i);
            						}
            					})
        	...
        	document.onclick = function(e){
        		if(!layui.$(e.target).parents('#'+filter+'-menu')[0])
        		pMenu.css({'display':'none'});
        	}
        	pMenu.on('click','li',function(){
        		var type = this.getAttribute('lay-event');
        		switch	(type) {
        			case 'refresh' : 
        				refreshNode(_that,options.refreshNode);
        				break;
        			case 'rename' : 
        				renameNode(_that,targetItem,options.renameNode);
        				break;
        			case 'create' : 
        				addNode(_that,{ text: "",  children: [] },options.deleteNode);
        				break;
        			case 'delete' :
        				deleteNode(_that,targetItem,options.deleteNode);
        				break;
        		}
	...
	var refreshNode = function(_this,callback){
		_this.render();
		'function' == typeof callback && callback(_this._dataJson) ;
	}
	
	var addNode = function(_this,data,callback){
		var oldData = _this._dataJson;
		layui.$.each(thisIndex,function(index,i){
			oldData[i].spread = 1;
			oldData = oldData[i].children;
		})
		layer.prompt({title:'请输入子节点名称'},function(val,index){
			data.text = val;
			layer.close(index);
			if(oldData){
				oldData.push(data);
			...
			pMenu.css({'display':'none'});
			_this.render();
			'function' == typeof callback && callback(_this._dataJson,thisIndex) ;
		})
		
	}
	
	var renameNode = function(_this,targetItem,callback){
		var oldData = _this._dataJson;
		lastIndex = thisIndex.pop();
		layui.$.each(thisIndex,function(index,i){
			oldData = oldData[i].children;
		})
		
		layer.prompt({title:'请输入子节点修改后的名称'},function(val,index){
			oldData[lastIndex].text = val;
			layui.$(targetItem).html(val);
			layer.close(index);
			pMenu.css({'display':'none'});
			'function' == typeof callback && callback(_this._dataJson,thisIndex) ;
		})
	...
	var deleteNode = function(_this,targetItem,callback){
		var oldData = _this._dataJson;
		var index = layer.confirm('确定删除此项？',function(){
			lastIndex = thisIndex.pop();
			layui.$.each(thisIndex,function(index,i){
				oldData = oldData[i].children;
			})
			layui.$(layui.$(targetItem).parents('.layui-xtree-item')[0]).remove();
			for(var j = lastIndex;j<oldData.length;j++){
				if(oldData[j+1] == undefined){
					oldData.pop();
				}else{
	...
	exports('layuiXtree', function (options) {
        return new layuiXtree(options);
    });
})