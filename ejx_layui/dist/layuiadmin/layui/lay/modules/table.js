...
/**
 * 2018/4/25 新增 根据多级dataName查找指定级数据
 */
function searchData(r,t,name){
		var data = new Object();
	...
	M.prototype.pullData = function(e, i) {
		...
		if (a.startTime = (new Date).getTime(), n.url) {
			...
			t.ajax({
				...
				headers: n.headers || {},
				success: function(t) {
//				2018/4/25 修改 根据多级dataName/countName 查找指定层
					try{
						countNameInfo = r.countName;
						dataNameInfo = r.dataName;
						t.newcountName = searchData(r,t,"countName");
						t.newdataName = searchData(r,t,"dataName");
					...
					r.countName = countNameInfo;
					r.dataName = dataNameInfo;
				},
				error: function(e, t) {
